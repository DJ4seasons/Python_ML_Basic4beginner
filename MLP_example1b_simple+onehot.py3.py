...
Test One-Hot encoding of label
...
def main():
    ...
    ### Label encoding (El Nino, Neutral, La Nina)
    one_std= 0.6 #np.std(nn34,ddof=1)
    y = np.digitize(nn34,[-one_std,one_std]) #-1
    nn_label=np.array(['La Ni\u00F1a','Neutral','El Ni\u00F1o'])
    ...
    lb_label= lb.classes_
    print("\nAfter One-Hot encoding, ",y.shape)
    print("E.g., ",y[0,:], y[100,:])
    ...
    plt.plot(mlp.loss_curve_)  ## Loss: a number how bad the model's prediction
    ...
    print("\nTraining Score")
    print(classification_report(lb.inverse_transform(y_train),lb.inverse_transform(y_pred0)))
    print("Confusion matrix, without normalization")
    print(confusion_matrix(lb.inverse_transform(y_train),lb.inverse_transform(y_pred0)).T)

    ## Test Score
    print("\nTest Score")
    y_pred= mlp.predict(X_test)
    print(classification_report(lb.inverse_transform(y_test),lb.inverse_transform(y_pred)))
    cm=confusion_matrix(lb.inverse_transform(y_test),lb.inverse_transform(y_pred)).T
    fns.plot_confusion_matrix(cm,labels=nn_label[lb_label])
    plt.show()
