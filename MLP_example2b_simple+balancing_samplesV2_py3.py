...
Test over-sampling, Ver2. adding white noise
...
def main():
    ...
    def generate_new_samplesV2(X1,y1,tgt_size,std_rate=0.2):
        """Simple duplication with white noise to increase samples"""
        ...
        X1_std= np.std(X1,axis=0,ddof=1)
        white_noise= np.random.uniform(low=-1.,high=1.,size=(size2add,X1.shape[1]))*X1_std[None,:]*std_rate
        return X1[idx2copy,:]+white_noise,y1[idx2copy]

    tgt_size= (y_train==0).sum()
    ## La Nina case
    idx= y_train==0
    X_add1, y_add1= generate_new_samplesV2(X_train[idx,:],y_train[idx],tgt_size)
    ## El Nino case
    idx= y_train==2
    X_add2, y_add2= generate_new_samplesV2(X_train[idx,:],y_train[idx],tgt_size)
    ## Concatenate
    print("\nBefore balancing")
